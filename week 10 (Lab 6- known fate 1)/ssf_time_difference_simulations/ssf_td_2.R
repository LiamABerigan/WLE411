...
ssf_td_2 <- function(time_difference, selected_season) {
  ...
  freq_nest$steps <- map(freq_nest$steps, function(x){
    ...
    x$season[1] <- case_when(
      x$step_state_start[1] == "Step state: Migratory (spring)" ~ "Migratory (spring)", #begins as a spring mig stopover point
      x$step_state_start[1] == "Step state: Migratory (fall)" ~ "Migratory (fall)", #begins as a fall mig stopover point
  ...
  freq_spring_mig_2 <- freq_nest
  
  freq_spring_mig_2$num_steps <- map(freq_spring_mig_2$steps, function(x){
    x %>% 
      filter(order_2_burst == selected_season) %>% 
      nrow()
  })
  ...
  # definitely removing for the 2nd order determination, as it's not going to be reasonable for summer or winter delineations
  
  freq_spring_mig_2 <- freq_spring_mig_2 %>% 
    filter(num_steps > 0)
  ...
  pop_sl_distr <- freq_spring_mig_2 %>% 
  ...
  pop_ta_distr <- freq_spring_mig_2 %>% 
    dplyr::select(local_identifier, steps) %>% 
    unnest(cols = c(steps)) %>% 
    mutate(dt_ = as.numeric(dt_)) %>% 
    filter(dt_ <= time_difference) %>% 
  ...
  freq_spring_mig_2$ua <- map(freq_spring_mig_2$steps, function(x){
  ...
  freq_spring_mig_2$nrow_ua <- map(freq_spring_mig_2$ua, function(x){
    nrow(x)
  })
